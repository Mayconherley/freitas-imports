...
@section('content')
<section class="mx-auto grid max-w-5xl gap-5 px-4 py-8 sm:px-6 lg:grid-cols-2 lg:px-8">
    <div class="rounded-lg bg-white p-6 ring-1 ring-black/10 sm:p-8">
        <h1 class="text-3xl font-bold">Minha conta</h1>
        <p class="mt-2 text-sm text-[#756f66]">Entre para acompanhar pedidos e agilizar suas proximas compras.</p>
        <form action="{{ route('customer.login.store') }}" method="POST" class="mt-6 space-y-4">
            ...
            <button class="w-full rounded-lg bg-[#06166f] px-5 py-3 text-sm font-bold text-white">Acessar minha conta</button>
        </form>
    </div>

    <div class="rounded-lg bg-white p-6 ring-1 ring-black/10 sm:p-8">
        <h2 class="text-3xl font-bold">Criar conta</h2>
        <p class="mt-2 text-sm text-[#756f66]">Cadastre-se para comprar com mais praticidade e guardar seu historico.</p>
        <form action="{{ route('customer.register') }}" method="POST" class="mt-6 space-y-4">
            ...
            <button class="w-full rounded-lg bg-[#d40000] px-5 py-3 text-sm font-bold text-white">Criar minha conta</button>
        </form>
    </div>
</section>
@endsection
